...
local function DebugPrint(...)
    if debugMode then
        print("|cffcc0000DK Assist [DEBUG]:|r", ...)
    end
end

function addon:ToggleDebug()
    debugMode = not debugMode
    print("|cffcc0000DK Assist:|r Debug mode " .. (debugMode and "ON" or "OFF"))
end
...
function addon:ScanCDMSafe()
    if not DKAssistDB then return 0 end
    if not addon.CreateCDMOverlaysAdditive then return 0 end
    if not DKAssistDB.trackCDMFestering and not DKAssistDB.trackCDMPutrefy then
        return 0
    end
